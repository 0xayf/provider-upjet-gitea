...
// Setup wires the controller for cluster-scoped Token resources.
...
// scopesEqual compares two scope sets ignoring order.
...
// Update reconciles scope drift by deleting the existing token and minting
// a new one. The new token's value flows through the connection secret;
// downstream consumers (a repo ActionsSecret with hash-rotation, an ESO
// ExternalSecret in dockerconfigjson form) pick up the new value
