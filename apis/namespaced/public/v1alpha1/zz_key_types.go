...
type KeyInitParameters struct {
	...
	// (String, Sensitive) An armored SSH key to add
	// An armored SSH key to add
	KeySecretRef v1.LocalSecretKeySelector `json:"keySecretRef" tf:"-"`

	// (Boolean) Describe if the key has only read access or read/write
	// Describe if the key has only read access or read/write
	ReadOnly *bool `json:"readOnly,omitempty" tf:"read_only,omitempty"`

	// (String) Title of the key to add
	// Title of the key to add
	Title *string `json:"title,omitempty" tf:"title,omitempty"`

	// (String) User to associate with the added key
...
// Key is the Schema for the Keys API. gitea_public_key manages ssh key that are associated with users.
